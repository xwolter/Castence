...
import {
    MessageCircle, Minimize2, Send, Plus, Hash, Lock, X, ArrowLeft, Trash2, Smile, ExternalLink, BarChart2
} from "lucide-react";
...
export default function GlobalChat({ user, userRole }: GlobalChatProps) {
    ...
    const [reactingToMsgId, setReactingToMsgId] = useState<string | null>(null); // ID wiadomości, którą reagujemy
    ...
    const handleCreatePoll = async (e: React.FormEvent) => {
        ...
        const validOptions = pollOptions.filter(o => o.trim() !== "").map((opt, index) => ({
            id: index,
            text: opt,
            votes: [] // Tablica UID osób głosujących
        }));
    ...
    const handleVote = async (msgId: string, optionId: number, currentOptions: any[]) => {
        ...
        const newOptions = currentOptions.map(opt => {
            // Jeśli to kliknięta opcja:
            if (opt.id === optionId) {
                // Jeśli już głosował, zabierz głos. Jeśli nie, dodaj.
                ...
                // Z innych opcji zabieramy głos (żeby można było głosować tylko na jedno)
                return { ...opt, votes: opt.votes.filter((uid: string) => uid !== user.uid) };
            }
        });
    ...
    const onEmojiClick = async (emojiObject: any) => {
        // Jeśli tryb dodawania reakcji do wiadomości
        if (reactingToMsgId && activeChannelId) {
            ...
            // Toggle: jak już dałem to zdejmij, jak nie to dodaj
            if (users.includes(user.uid)) {
                users = users.filter((uid: string) => uid !== user.uid);
            } else {
                users.push(user.uid);
            }

            // Jeśli pusta lista, usuń klucz, inaczej zaktualizuj
            ...
            setReactingToMsgId(null); // Zamknij picker po wyborze
            setShowEmoji(false);
        } else {
            // Zwykłe dodawanie do inputa
                                ...
                                return (
                                    <div key={msg.id} className={`flex gap-3 ${isMe ? 'flex-row-reverse' : ''} group/msg`}>
                                        ...
                                        <div className={`max-w-[80%] relative flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                                ...
                                                <div className="w-64 bg-neutral-900 border border-neutral-700 rounded-xl p-3">
                                                    ...
                                                    <div className="space-y-2">
                                                        {msg.options.map((opt: any) => {
                                                            ...
                                                            return (
                                                                <div key={opt.id} onClick={() => handleVote(msg.id, opt.id, msg.options)} className="cursor-pointer group/opt">
                                                        ...
                                                        <div className="flex flex-wrap gap-1 mt-1.5 pt-1 border-t border-white/10">
                                                            {Object.entries(msg.reactions).map(([emoji, users]: any) => (
                                                                <button
                                                                    ...
                                                                    className={`text-[10px] px-1.5 py-0.5 rounded-full flex items-center gap-1 ${users.includes(user.uid) ? 'bg-indigo-500/30 text-white border border-indigo-500/50' : 'bg-black/20 text-neutral-300 hover:bg-black/40'}`}
                                                                >
                                                                    {emoji} {users.length}
                                                                </button>
                                                            ))}
                                            ...
                                            {/* MENU AKCJI (KOSZ I REAKCJA) */}
                                            <div className={`absolute top-0 flex gap-1 opacity-0 group-hover/msg:opacity-100 transition ${isMe ? '-left-14' : '-right-14'}`}>
                                                <button
                                                    onClick={() => { setReactingToMsgId(msg.id); setShowEmoji(true); }}
                                                    className="p-1.5 bg-neutral-900 border border-neutral-700 rounded-full text-neutral-400 hover:text-yellow-400 shadow-md"
                                                >
                                                ...
                                                {canDelete && (
                                                    <button
                                                        ...
                                                        className="p-1.5 bg-neutral-900 border border-neutral-700 rounded-full text-neutral-400 hover:text-red-500 shadow-md"
                                                    >
                                ...
                                {isPollMode ? (
                                    ...
                                    <div className="bg-[#161616] border border-neutral-800 rounded-xl p-3 animate-in slide-in-from-bottom-2">
                                        <div className="flex justify-between mb-2">
                                            <span className="text-xs font-bold text-indigo-400 uppercase">Tworzenie Ankiety</span>
                                            <button onClick={() => setIsPollMode(false)}><X className="w-4 h-4 text-neutral-500 hover:text-white"/></button>
                                        </div>
                                        ...
                                        <div className="space-y-1 max-h-24 overflow-y-auto mb-2">
                                            {pollOptions.map((opt, idx) => (
                                                <input key={idx} className="w-full bg-neutral-900 border border-neutral-800 rounded p-1.5 text-xs text-white" placeholder={`Opcja ${idx+1}`} value={opt} onChange={e => {
                                                    const newOpts = [...pollOptions]; newOpts[idx] = e.target.value; setPollOptions(newOpts);
                                                }} />
                                            ))}
                                    ...
                                    <form onSubmit={handleSendMessage} className="flex gap-2 items-center">
                                        {userRole === 'admin' && (
                                            <button type="button" onClick={() => setIsPollMode(true)} className="p-2 rounded-lg text-neutral-400 hover:text-white hover:bg-neutral-800 transition" title="Ankieta">
                                                <BarChart2 className="w-5 h-5" />
                                            </button>
                                        )}
                                        <button type="button" onClick={() => { setReactingToMsgId(null); setShowEmoji(!showEmoji); }} className={`p-2 rounded-lg transition ${showEmoji ? 'text-yellow-400' : 'text-neutral-400 hover:text-white'}`}>
